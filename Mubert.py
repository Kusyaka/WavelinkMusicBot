...
class Mubert:
    def __init__(self):
        ...
        self._mubert_tags = np.array(mubert_tags_string.split(','))
        self._mubert_tags_embeddings = self._minilm.encode(self._mubert_tags)
    ...
    def _get_track_by_tags(self, tags, duration, maxit=20, autoplay=False, loop=False):
        if loop:
            mode = "loop"
        else:
            mode = "track"
    ...
    def generate_track_by_prompt(self, prompt, duration, loop=False):
        _, tags = self._get_tags_for_prompts([prompt, ])[0]
        try:
            return tags, self._get_track_by_tags(tags, duration, autoplay=True, loop=loop)
...
# M = Mubert()
# print(M.generate_track_by_prompt("Oh he decisively impression attachment friendship so if everything. ",
#                                  duration=30))
